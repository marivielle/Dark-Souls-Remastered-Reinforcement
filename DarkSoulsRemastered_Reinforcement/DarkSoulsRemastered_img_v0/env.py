...
class DarkSoulsEnv(Env):
    def __init__(self):
        #actions we can take
        self.screen = GrabScreen()
        self.action_space = Discrete(10)
        self.observation_space = Box(low=0, high=225, shape=(self.screen.res_w, self.screen.res_h, 4), dtype=np.uint8)
        ...
        self.image_stack = []
        self.img_stack_len = 5
        self.state = self.screen.capture()
        self.info = self.set_current_state(True)
        self.previous = {'Boss Health': 0, 'Player Health': 0, 'Location Difference': 0}

    def set_current_state(self, reset):
        self.image_stack = []

        while len(self.image_stack) < self.img_stack_len:
            img = self.screen.capture()
            self.image_stack.append(img)
            time.sleep(0.02)

        self.state = np.stack(self.image_stack)
    ...
    def step(self, action):
        ...
        self.info = self.set_current_state(False)
        ...
        if self.info['Boss Health'] < self.previous['Boss Health']:
            reward += 1  #prioritise boss losing health over us losing health
