...
class Agent:
    def __init__(self, action_size, state_size, dims):
        ...
        self.learning_rate = 0.0001 #added one more 0
        self.epsilon = 1.0
    ...
    def build_model(self):
        ...
        model.add(Conv2D(32, 7,
                         strides=(4, 4),
                         kernel_initializer=tf.keras.initializers.variance_scaling(scale=2),
                         input_shape=self.state_size, activation='relu', name="C1"))
        #model.add(MaxPool2D(pool_size=(2, 2)))

        model.add(Conv2D(64, 7, strides=(4, 4),
                         kernel_initializer=tf.keras.initializers.variance_scaling(scale=2),
                         activation='relu', name="C2"))  #MAKE THIS THE STATE_SIZE
        #model.add(MaxPool2D(pool_size=(2, 2)))

        model.add(Conv2D(64, 3, strides=(2, 2), kernel_initializer=tf.keras.initializers.variance_scaling(scale=2),
                         activation='relu', name="C3"))  #what's padding
        #model.add(MaxPool2D(pool_size=(2, 2)))

        model.add(Conv2D(128, 3, strides=(1, 1), kernel_initializer=tf.keras.initializers.variance_scaling(scale=2),
                         activation='relu', name="C4"))  #wtf is strides lmao
        #model.add(MaxPool2D(pool_size=(2, 2)))
