...
class DarkSoulsEnv(Env):
    def __init__(self):
        ...
        self.observation_space = Box(low=np.array([0, 0, 0, 0, 0]), high=np.array([3432, 1459, 9999999999, 9999999999, 9999]))
    ...
    def step(self, action):
        ...
        if self.info['Location Difference'] < self.previous['Location Difference']:
            reward += 0.1  #encourage decisions that lead to player being closer to the boss
        ...
        if self.info['Boss Health'] < self.previous['Boss Health']:
            reward += 0.6  #prioritise boss losing health over us losing health
